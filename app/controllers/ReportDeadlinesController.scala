...
class ReportDeadlinesController @Inject()(val authentication: AuthenticationPredicate,
  ...
  def getReportDeadlines(nino: String): Action[AnyContent] = authentication.async { implicit request =>
    Logger.debug(s"[ReportDeadlinesController][getReportDeadlines] - Requesting Estimate from EstimatedTaxLiabilityService for NINO: $nino")
    reportDeadlinesService.getBusinessDeadlines(nino).map {
